...
void direct(int n, double *lb, double *ub, int maxint, double fglob, double *xbest, double *fbest, double funct(int, double*), 
			int *Number_of_experiments, int Function_number) {
	...
	*fbest = funct(n, xbest, Function_number);
	fdir        = *fbest;
	primo->fint = fdir;
	...
	while(!halt){
		...
		while (currch != NULL){
			...
			if(currch->inter->flagdiv) 
				suddividi(currch->inter, n, &nf, &nint, xdir, &fdir, funct, Function_number);
			currch = currch->next;
		}
...
void suddividi(intervallo *curr, int n, int *nf, int *nint, double *xdir, double *fdir, double funct(int, double*), int Function_number) {
	int i, j;
	...
	for(i=0;i<n;i++){
		...
		if(curr->maxdim == curr->dimen[i]){
			...
			mod_suddividi.vetf1[i] = funct(n, mod_suddividi.xsud, Function_number);

			mod_suddividi.ysud[i] = curr->cent[i] - 1.0*curr->dimen[i]/3.0;
			unscalevars(n,mod_suddividi.ysud,mod_suddividi.xsud);
			mod_suddividi.vetf2[i] = funct(n, mod_suddividi.xsud, Function_number);
